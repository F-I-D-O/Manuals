...
For each workspace, there are two files:

- `workspace.dsl`: we edit this file to define the model and views
- `workspace.json`: this file is generated by the server, it contains compiled information from the `workspace.dsl` file and some additional information like specific positioning of the elements.
...
```dsl
<id> = <type> "<description>" {<children>}
```

The `<children>` are optional. The `<type>` can be:

- `softwaresystem`: level 1
- `container`: level 2
- `component`: level 3
- `code`: level 4
- `person`: level 1
...
- [`include`](https://docs.structurizr.com/dsl/language#include): what to include in the diagram.
- [`exclude`](https://docs.structurizr.com/dsl/language#exclude): what to exclude from the diagram.
- `title`: the title of the diagram.

The inclusion logic is as follows:

- nothing is included by default
- connections between included nodes are included automatically
- excluded elements are not included, overriding inclusion

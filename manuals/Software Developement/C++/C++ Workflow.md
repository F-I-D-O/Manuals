# Introduction
In C++, the workflow and especially the build pipeline is much more complicated than in other languages. Therefore, we start with brief overview of the C++ build pipelines. The following scheme shows the possible build pipelines for C++:

![C++ Workflow](https://fido.ninja/diagrams/Build%20automation.svg)

This guide discusses only some of the build pipelines.
...
- Optimization level: Specify the optimization level of the binary. There are either:
	- [high-level options](https://learn.microsoft.com/en-us/cpp/build/reference/o1-o2-minimize-size-maximize-speed):
		- `/O1`: optimize size
		- `/O2`: optimize speed
	- or more detailed low-level options:
		- [`/Ob`](https://learn.microsoft.com/en-us/cpp/build/reference/ob-inline-function-expansion): specified with level option immediately after the `/Ob` flag, e.g., `/Ob2`. Controls inlining. Options:
			- `0`: Disable inlining.
			- `1`: Only function marked with `inline` and class member functions defined in the class declaration are inlined.
			- `2`: Any function can be inlined.
			- `3`: Like `2`, but more aggressive inlining.
- [Preprocessor definitions](https://learn.microsoft.com/en-us/cpp/build/reference/d-preprocessor-definitions): syntax `/D[ ]<macro>[=<value>]`, e.g., `/D WIN32` or `/DWIN32`. Frequently used macros are:
	- `NDEBUG`: disable asserts and debug only code
	- `WIN32`: historical macro for signaling that the code is compiled for Windows. Only lagacy code should use this macro. However, it is still set by CMake for compatibility reasons in any Windows build scripts.
	- `_WINDOWS`: historical macro for signaling that the executable is a Windows GUI application. Only lagacy code should use this macro.
- [Source file type specification](https://learn.microsoft.com/en-us/cpp/build/reference/tc-tp-tc-tp-specify-source-file-type?view=msvc-170)
	- `/Tc`: C source file
	- `/Tp`: C++ source file
...
Build sequencing tools are tools that takes a build script as an input, and calls several compiler toolchain tools like compiler or linker, to generate the final executable.
...
## MSBuild
The MSBuild XML-based build sequencing tool. As it is a multi-language tool, it is described in a [separate manual](../Common.md#msbuild). Only the C++ specifc things are ddescribed here.

The MSBuild scripts are typically generated by CMake or directly by Visual Studio. The main file for each target, the *project file*, is named `<target name>.vcxproj` file.

All source files for compilation are listed as `<ClCompile>` elements under the `ItemGroup` element. These elements contain all properties for the compilation (e.g., language standard, runtime library, etc.)

Besides that, there can be other elements:

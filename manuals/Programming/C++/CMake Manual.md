...
- `--graphviz=<file name>` to generate a graphviz file of the dependencies. Note that only visible CMake dependencies are included
...
## Rebuild or Clean the build binaries
To rebuild, we use the `--clean-first` argument:
```bash
cmake --build . --clean-first
```

If we want just to clean the build binaries without building, we use the `clean` target:
```bash
...
### Lists
[documentation](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#lists)

In CMake, strings are interpreted as lists in many contexts if they contain semicolons (`;`), which serves as a list delimiter. For example, if an unquoted argument contains a semicolon, it is interpreted as a list of arguments.

List variables are defined similarly to scalar variables using the `set` command:
```cmake
set(<name> <value 1> <value 2> ...) # stores `<value 1>;<value 2>;...` in the `<name>` variable
```

If we need to use a string containing a semicolon in context that evaluates lists, we need to escape it using `\;`.

When working with lists, it is important to think about how we want to use them:

- many commands accept list arguments -> we want to pass the list as a single argument, quoted
    - commands that accept lists (e.g., `add_executable`, `add_library`, `target_link_libraries`)
- in other contexts, we want to pass the list as a list of arguments, unquoted
    - for loops
...
There are some variable generated by default by CMake. Howver, these differ between the *script* and *config* modes. 

#### Variables available in both modes

- [`CMAKE_BINARY_DIR`](https://cmake.org/cmake/help/latest/variable/CMAKE_BINARY_DIR.html): the top level build directory. Most of the time, it is appropriate to use `CMAKE_CURRENT_BINARY_DIR` instead of this variable.
    - in the *script* mode, this variable is set to the directory where the script is located.
- [`CMAKE_CURRENT_BINARY_DIR`](https://cmake.org/cmake/help/latest/variable/CMAKE_CURRENT_BINARY_DIR.html): the directory where the build scripts for the current target are located and where the build process is executed. For some generators, this is also the directory where the binaries are stored.
    - in the *script* mode, this variable is set to the directory where the script is located.
- [`CMAKE_CURRENT_FUNCTION_LIST_DIR`](https://cmake.org/cmake/help/latest/variable/CMAKE_CURRENT_FUNCTION_LIST_DIR.html): the directory where the function or macro that is currently being processed is located.
- [`CMAKE_CURRENT_FUNCTION_LIST_FILE`](https://cmake.org/cmake/help/latest/variable/CMAKE_CURRENT_FUNCTION_LIST_FILE.html): the path to the file that contains the function or macro that is currently being processed.
- [`CMAKE_CURRENT_LIST_FILE`](https://cmake.org/cmake/help/latest/variable/CMAKE_CURRENT_LIST_FILE.html): the path to the currently processed `CMakeLists.txt` file `*.cmake` file. 
    - does not consider function and macro calls. If you want to know the path to the file that contains the current function or macro, use `CMAKE_CURRENT_FUNCTION_LIST_FILE`.
- [`CMAKE_CURRENT_SOURCE_DIR`](https://cmake.org/cmake/help/latest/variable/CMAKE_CURRENT_SOURCE_DIR.html): the directory where the currently processed `CMakeLists.txt` file is located.
    - in the *script* mode, this variable is set to the directory where the script is located.
- [`CMAKE_SOURCE_DIR`](https://cmake.org/cmake/help/latest/variable/CMAKE_SOURCE_DIR.html): the directory where the top level `CMakeLists.txt` file is located. This variable is rarely the right one, because our project may be used as a subproject in another project.
    - in the *script* mode, this variable is set to the directory where the script is located.


#### Config mode variables

- [`CMAKE_<LANG>_COMPILER`](https://cmake.org/cmake/help/latest/variable/CMAKE_LANG_COMPILER.html): the path to the compiler for the language `<LANG>`. Either a full path or a relative path if we use a build environment like a VS PowerShell environment.
...
- [`CMAKE_EXPORT_COMPILE_COMMANDS`](https://cmake.org/cmake/help/latest/variable/CMAKE_EXPORT_COMPILE_COMMANDS.html): set to `ON` if we want to export the compile commands. Only works for Makefile and Ninja generators.
- [`CMAKE_PROJECT_NAME`](https://cmake.org/cmake/help/latest/variable/CMAKE_PROJECT_NAME.html): the name of the top level project. Most of the time, it is appropriate to use `PROJECT_NAME` instead of this variable.
- [`MSVC`](https://cmake.org/cmake/help/latest/variable/MSVC.html): set to `TRUE` if the compiler is MSVC. Useful for compiler specific branching in `CMakeLists.txt`.
- [`PROJECT_NAME`](https://cmake.org/cmake/help/latest/variable/PROJECT_NAME.html): the name of the project.
...
## Escape sequences
[documentation](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#escape-sequences)

Escape sequences are used to escape special characters in strings. In CMake, there are the following escape sequences:

- `\n`: new line
- `\r`: carriage return
- `\t`: tab
- `\<non-alphanumeric character>`: the character itself


## Command Arguments and Literals
[documentation](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#command-arguments)

In CMake, there is nothing like assignment, event a simple variable setting is done using a command. Therefore, literals are not considered at all in CMake language, instead, CMake defines *command arguments* as the basic building blocks.

There are three types of command arguments, mostly different in whether they evaluate variables, [escape sequences](#escape-sequences), or whether they perform list division:

- [unquoted arguments](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#unquoted-arguments): continous text without special characters
    - The following characters need to be escaped: `(`, `)`, `#`, `;`, `\`, or any whitespace character.
    - the resulting value is divided using the list logic. Therefore, `;` needs to be escaped as well, if we do not want to pass a list.
    - escape sequences and variables are evaluated.
- [quoted arguments](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#quoted-arguments): text surrounded by double quotes
    - unlike unquoted arguments, quoted arguments may contain unescaped whitespace characters
    - if we escape a new line character, it is interpreted as a line-continuation (basically, no line break is inserted)
    - the resulting value is not divided using the list logic.
    - empty string (`""`) is valid (not possible for unquoted arguments)
    - otherwise, the behavior is the same as for unquoted arguments
- [bracket arguments](https://cmake.org/cmake/help/latest/manual/cmake-language.7.html#bracket-arguments): text surrounded by special delimiter: `[=*[` for opening and `]=*]` for closing (where `*` means that the `=` may appear any number of times)
    - no characters need to be escaped
    - escape sequences and variables are evaluated.
    - no list division is performed.

Note that we need to chose a single argument type for the whole argument. Example:
```cmake
EXPECTED=Hello # fine a single unquoted argument
EXPECTED="Hello" # wrong
"EXPECTED=Hello" # fine, single quoted argument
EXPECTED=[==[Hello \nWorld]==] # wrong
```
...
The `<condition>` result is a boolean value:

- `FALSE`, `""`, `0`, `NOTFOUND`, `OFF`, `NO`, `N`, `IGNORE`, or string ending with `-NOTFOUND` evaluate to `FALSE`
...
- string operators: `MATCHES`, `LESS`, `GREATER`, `LESS_EQUAL`, `GREATER_EQUAL`, `STREQUAL`, `STRLESS`, `STRGREATER`, `STRLESS_EQUAL`, `STRGREATER_EQUAL`
    - The `MATCHES` operator requires a regex, not a simplified filesystem filter pattern. See the [regex documentation](#regular-expressions) for more.
...
## Properties
Besides setting variables, we can set properties that are bound to specific entities in the build system.

- For setting properties, we use the [`set_property`](https://cmake.org/cmake/help/latest/command/set_property.html) command:
    ```cmake
    set_property(<scope> <target/directory/file/variable> PROPERTY <property name> <property value>)
    ```
- For getting properties, we use the [`get_property`](https://cmake.org/cmake/help/latest/command/get_property.html) command.
    ```cmake
    get_property(<scope> <target/directory/file/variable> PROPERTY <property name> <output variable>)
    ```

Here, the `<scope>` is the type of the entity to which the property is bound. It can be:
...
We can use these properties for our custom variables, e.g., if we want to bound them to a specific target. However, [lot of entities have a specific set of properties predefined](https://cmake.org/cmake/help/latest/manual/cmake-properties.7.html).


For properties bound to targets, we can use dedicated commands:

- [`set_target_properties`](https://cmake.org/cmake/help/latest/command/set_target_properties.html)
    ```cmake
    set_target_properties(<target names>... PROPERTIES <property names> <property values>)
    ```
- [`get_target_property`](https://cmake.org/cmake/help/latest/command/get_target_property.html)
    ```cmake
    get_target_property(<output variable> <target name> <property name>)
    ```
...
- the **syntax is limited** compared to the usual regex engines,
- the **usage is limited** as well,
- many **things work contrary to the expected behavior**, and
- the regexes in CMake are **poorly documented**


We can use regexes in these contexts:

- in some `string` commands (see [String commands](#string-commands) section),
- in the `if` control structure, when the `MATCHES` operator is used.

The CMake [regex syntax](https://cmake.org/cmake/help/latest/command/string.html#regex-specification) is limited to following special characters:

- `.`: any character
- `*`, `+`, and `?`: zero or more, one or more, and zero or one quantifiers
- `[]`: character class (matching literals inside the brackets). 
    - A range can be used inside the brackets, specified by a hyphen, e.g., `[a-z]` matches any lowercase letter.
    - The negation (`^`) can be used to match any character except the ones inside the brackets.
- `^` and `$`: anchor to the start and end of the string
- `|`: OR operator
- `()`: capture group
- `\`: escape character for matching any of the special characters literally


**No other special characters are supported.**

To match whitespace characters, we can use the CMake escape sequences. However, we need to make sure that they will be evaluated before the string is processed by the regex engine:

- `if("TESTSTRING\n" MATCHES ".*\n")` works, escape sequence is evaluated by cmake
- `if("TESTSTRING\n" MATCHES ".*\\n")` does not work, sequence `\n` is processed by the regex engine, which interprets it as sequence of two characters: `\` and `n` literally
- `if("TESTSTRING\n" MATCHES [=[.*\n]=])` does work, from the same reason as the previous case.
...
## String commands
[documentation](https://cmake.org/cmake/help/latest/command/string.html)

String commands have the following syntax:
```cmake
string(<command> <arguments>)
```
Where typically, the last arguments are `<output variable>` and `<input>`, if present, in this order.

Notable string commands:

- `REGEX MATCH <regex> <output variable> <input>`: matches the regex against the input and stores the result in the output variable. 
    - see the [regular expressions](#regular-expressions) section for information about the regex syntax.
...
To find out which of the operation modes is used, the `find_package` command uses the following logic:
...
#### Versions
We may specify the version of package by using the `VERSION` parameter. The syntax is:

- `VERSION <minimum version>`
- `VERSION <minimum version>...<maximum version>`

The version here is the cmake version used in the cmake version file (i.e., `<package name>-config-version.cmake` or `<package name>ConfigVersion.cmake`) of the package. Typically, this is the project version of the package, specified in the `project` command.
...
1. Download the dependency using the `FetchContent_Declare` command
    ...
    - The `<SPECIFICATION>` can be:
        - `URL <url>` with and optional `HASH <hash>`
        - `GIT <git repository>` with an optional `TAG <tag>` or `BRANCH <branch>` or `COMMIT <commit>`
...
### Set preprocessor variables for a target
To set preprocessor variables for a target, use the [`target_compile_definitions`](https://cmake.org/cmake/help/latest/command/target_compile_definitions.html) command. The syntax is:
```cmake
target_compile_definitions(<target name> <SCOPE> <definition 1> <definition 2> ...)
```

Note that there is no good way to set list variables this way, as the preprocessor does not support lists. To pass lists of variables, use generated header files (see [generating files based on templates](#generating-files-based-on-templates)).
...
### Fixing incorrect linking configuration in upstream projects
Sometimes, an upstream project may incorrectly link a library as a private dependency, despite the fact that it is needed at runtime by the downstream project. We can fix it by set up the linking in our project:

```cmake
target_link_libraries(<upstream target> INTERFACE <library name>)
```

Of course, the `<library name>` has to be known, i.e., we may need to also add the appropriate `find_package` command to our project.
...
Sometimes, we can have dependencies between targets other than the linking dependencies. For example, our test target may use a functional test, and thus it depends on the executable target. To ensure that such a dependency is built before the test target, we can use the [`add_dependencies`](https://cmake.org/cmake/help/latest/command/add_dependencies.html) command. The syntax is:
```cmake
add_dependencies(<target name> <dependencies>)
```
Here, the `<dependencies>` is a list of target names.
...
#### The package configuration file and the version file 
...
The version file is generated using the [`write_basic_package_version_file`](https://cmake.org/cmake/help/latest/module/CMakePackageConfigHelpers.html#generating-a-package-version-file) command. The syntax is:
```cmake
...
The syntax of the `add_subdirectory` command is:
```cmake
add_subdirectory(<source dir> [<binary dir>])
```
By default, the `<binary dir>` is the same as the `<source dir>`. 
...
## Generating files based on templates
[documentation](https://cmake.org/cmake/help/latest/command/configure_file.html)

For passing variables from the configuration scripts to C++ code, the [`target_compile_definitions`](#set-preprocessor-variables-for-a-target) command is typically used. However, if we need multiple variables, or lists of variables, the better approach is to skip the C++ preprocesser and generate a C++ header directly. Typically a template with patterns to be replaced is prepared, and the configuration script fill the patterns with the actual values. In CMake, there is a dedicated command for this purpose:

```cmake
configure_file(<input> <output>)
```
Here, the `<input>` is the path to a template file, the `<output>` is the output file path.

Other important parameters:

- `@ONLY`: only replace pattern of this syntax: `@<variable name>@`. Otherwise, even `${<variable name>}` is considered as a pattern.
...
- [`configurePresets`](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html#configure-preset): an array of configure presets objects. Each object has the following important properties:
    - `name` (required): the name of the preset.
    - `toolchainFile`: equals to the `--toolchain` argument of the `cmake` command.
    - `generator`: equals to the `-G`/ `--generator` argument of the `cmake` command.
...
## Presets Inheritance
Both `configurePresets` and `buildPresets` can inherit from other presets:
...
We can also inherit from more than one preset:
...
# CMake Policies
[documentation](https://cmake.org/cmake/help/latest/manual/cmake-policies.7.html)

To control the changes in cmake behavior, it uses the policies. With CMake policies, we can determine whether to use the old behavior (default) or the new behavior.
This way, backward compatibility is ensured, even when there are radical changes in the behavior.

If a policy is not explicitly set (see below), the *old* behavior is used, and a warning is issued.

For project owners, there are many multiple ways how to set the policies:

- using the [`cmake_minimum_required` command](https://cmake.org/cmake/help/latest/command/cmake_minimum_required.html), we are saying that all policies introduced in the specified minimum version and older are set to *new* behavior.
    - this should be the way how to set the policies, unless there are strong reasons to not use the new behavior (costly migration, etc.)
- using the [`cmake_policy` command](https://cmake.org/cmake/help/latest/command/cmake_policy.html), we can set a specific policy.


## `cmake_minimum_required` command
[documentation](https://cmake.org/cmake/help/latest/command/cmake_minimum_required.html)
...
Here the version can be:

- a single version number, e.g., `3.22`, in this case, all policies introduced in the version 3.22 and older are set to *new* behavior. Older cmake versions are not supported.
- a range of version numbers, e.g., `3.22...3.24`, in this case, all policies introduced in the version 3.24 and older are set to *new* behavior. However, the project has a backward compatibility with versions 3.22 and newer (i.e., for version between 3.22 and 3.24, both *old* and *new* behavior should work for all policies).


## `cmake_policy` command
[documentation](https://cmake.org/cmake/help/latest/command/cmake_policy.html)
...
Here the `<policy>` is in a form of `CMP<NNNN>`, where `NNNN` is the policy number. The `<value>` is either `OLD` or `NEW`.

Note that in general, the policies should be set using this command only temporarily. Warning may be issued after long time when the policy is set this way:

- if the policy is set to `OLD`, CMake versions released at least 2 years after the policy was introduced will raise a warning.
- if the policy is set to `NEW`, CMake versions released at least 5 years after the policy was introduced will raise a warning.
...
To setup testing in a project, we have to:
...
The **test result** is determined in a very simple way:

- if the test command exits with 0, the test is considered passed
- otherwise, the test is considered failed

To get a more sophisticated test result determination, we have two options:

- set test properties - see [Setting test properties](#setting-test-properties) (only for very simple tests), or
- implement the testing logic, including assertion, in the test command itself. For that purpose, we can set `<command>` to:
    - cmake script,
    - python script,
    - C++ code,
    - or any other executable, e.g., bash script
...
### Troubleshooting
If any of the test phases crash, we should inspect the logs, as the command line typically does not show the specific error message.

- when using the dashboard mode, there should be a link in the dashboard to the specific test log.
...
We can modify the test (e.g.: check the output of the test) using the [`set_property`](https://cmake.org/cmake/help/latest/command/set_property.html) command with the `TEST` scope:
```cmake
...
- [`PASS_REGULAR_EXPRESSION`](https://cmake.org/cmake/help/latest/prop_test/PASS_REGULAR_EXPRESSION.html#prop_test:PASS_REGULAR_EXPRESSION): a regular expression that the output of the test has to match:
    ...
    ```
- [`WILL_FAIL`](https://cmake.org/cmake/help/latest/prop_test/WILL_FAIL.html#prop_test:WILL_FAIL): if set to `TRUE`, the test is expected to fail.

[List of all test properties](https://cmake.org/cmake/help/latest/manual/cmake-properties.7.html#properties-on-tests)
...
## Using cmake scripts as test commands
CMake files can be used as test commands for the `add_test` command. Advantages:

- easy passing of project configuration parameters to the test script
- no compilation step needed (compared to C++)
- no additional dependencies (compared to Python)
- platform-independent (compared to shell scripts)

Disadvantages:

- no testing framework, everything has to be implemented
- CMake can be more cumbersome than a typical programming language

Therefore:

- cmake scripts are **very good for functional tests and smoke tests**, but
- cmake scripts are **not good for unit tests or integration tests**
- cmake scripts are **the only way how to test CMake functions**
